...
4. **Implement** -- Execute the approved plan. For TDD tasks, follow strict RED → GREEN → REFACTOR with verification at each step. Before requesting code review, verify: `npx tsc --noEmit` && `npm run build` && `npm test` all pass.
